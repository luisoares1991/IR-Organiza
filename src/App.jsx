...
function MainApp() {
  ...
  return <main className="min-h-screen w-full max-w-full overflow-x-hidden bg-[#f5f8f7] text-slate-950 dark:bg-slate-950 dark:text-slate-100">
    <div className="mx-auto flex min-h-screen w-full max-w-[1440px] overflow-x-hidden">
      ...
      <div className="min-w-0 max-w-full flex-1 overflow-x-hidden lg:ml-72"><div className="mx-auto min-h-screen w-full min-w-0 max-w-6xl overflow-x-hidden px-4 py-3 pb-28 sm:px-6 lg:px-8 lg:py-7 lg:pb-10">{screens[app.view] || screens.dashboard}</div></div>
    </div>
    <nav className="fixed inset-x-3 bottom-3 z-40 grid max-w-[calc(100vw-1.5rem)] grid-cols-[repeat(5,minmax(0,1fr))] rounded-[24px] border border-slate-200/80 bg-white/95 p-2 shadow-[0_16px_50px_rgba(15,23,42,.18)] backdrop-blur dark:border-slate-700 dark:bg-slate-900/95 lg:hidden">{navItems.map(([key,label,Icon]) => { const active=app.view===key; const add=key==='scan'; return <button key={key} onClick={() => go(key)} className={`relative flex min-h-14 min-w-0 flex-col items-center justify-center gap-1 overflow-hidden rounded-2xl text-[10px] font-bold ${add?'-mt-6':''} ${active?'text-teal-700 dark:text-teal-300':'text-slate-400'}`}>{add?<span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-teal-700 text-white shadow-lg shadow-teal-900/25">{React.createElement(Icon,{size:26})}</span>:React.createElement(Icon,{size:21})}<span className="max-w-full truncate px-0.5">{label}</span></button>})}</nav>
  </main>;
}
