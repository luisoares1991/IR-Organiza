...
export default function App() {
  ...
  const handleAnonymousLogin = async () => {
      if (confirm("⚠️ AVISO: No modo visitante, seus dados ficam salvos apenas neste navegador. Se limpar o cache, você perde tudo. Deseja continuar?")) {
  ...
  const analyzeWithGemini = async (base64, mimeType) => {
    ...
    try {
      ...
      alert(`Erro na IA: ${e.message}. Preencha manualmente.`);
  ...
  const handleSave = async () => {
    if (!reviewData.valor || !reviewData.razao_social) return alert("Preencha campos obrigatórios.");
    setLoading(true);
    try {
      const payload = {
        ...reviewData,
      ...
      let id = editingId;
      if (id) await updateDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'expenses', id), payload);
      else {
  ...
  const handleShare = async () => {
    if (!selectedExpenseImage) return;
  ...
  const handleExport = async () => {
    ...
    const data = { expenses: expSnap.docs.map(d => d.data()), dependents: depSnap.docs.map(d => d.data()) };
    ...
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 p-6 text-center">
        <div className="flex justify-center mb-8">
          {/* LOGO NO LOGIN */}
          <img src="/logo.png" alt="IR Organiza" className="w-32 h-32 rounded-3xl shadow-2xl shadow-blue-500/20" />
        </div>
        <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">IR Organiza</h1>
        <p className="text-slate-500 dark:text-slate-400 mb-8 max-w-xs">Seu assistente inteligente para organizar recibos.</p>
        <Button onClick={handleLogin} className="w-full max-w-xs py-4 text-lg shadow-xl mb-4"><LogIn size={24} /> Entrar com Google</Button>
        <button onClick={handleAnonymousLogin} className="text-slate-400 text-sm underline">Entrar sem conta (Modo Visitante)</button>
      </div>
    );
  }

  const totalDeductible = expenses.reduce((acc, cur) => acc + (cur.valor || 0), 0);
    ...
    <div className="space-y-6 pb-24">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
           ...
           <div><h1 className="text-xl font-bold text-slate-900 dark:text-white">Olá, {user.displayName?.split(' ')[0] || 'Visitante'}</h1><p className="text-slate-500 text-xs">Suas despesas</p></div>
        </div>
      ...
      <Card className="bg-gradient-to-br from-blue-600 to-indigo-700 text-white p-6 border-none">
        ...
        <div className="text-4xl font-bold mb-4">{formatCurrency(totalDeductible)}</div>
  ...
  const renderSettings = () => (
     <div className="pb-24 space-y-6">
        ...
        <div className="flex items-center gap-3 p-4 bg-slate-100 dark:bg-slate-900 rounded-xl">
           <div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold">{user.displayName?.[0] || 'V'}</div>
        ...
        {user.isAnonymous && <div className="bg-yellow-50 text-yellow-800 p-3 rounded-lg text-sm flex gap-2 items-center border border-yellow-200"><AlertCircle size={16}/> Dados temporários. Exporte ou faça login para salvar.</div>}
        ...
        <div className="text-center text-xs text-slate-400">v3.2 - {user.uid.slice(0,6)}</div>
     </div>
  );
