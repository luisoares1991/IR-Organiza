...
export default function App() {
  ...
  // SISTEMA DE NAVEGAÇÃO (Histórico)
  useEffect(() => {
    // Quando o usuário aperta o botão "Voltar" do celular
    const handlePopState = (event) => {
        if (event.state && event.state.view) {
            setView(event.state.view);
        } else {
            // Se acabou o histórico do app, volta pro dashboard ou deixa fechar se já estiver lá
            setView('dashboard'); 
        }
    };
  ...
  // Função para navegar empurrando histórico
  ...
  // Função para voltar (usada nos botões de seta)
  const goBack = () => {
  ...
  const handleAnonymousLogin = async () => {
      const confirmMsg = "⚠️ ATENÇÃO: MODO VISITANTE\n\nSeus dados não serão salvos na sua conta Google.\nSe você limpar o cache do navegador ou trocar de celular, perderá tudo.\n\nPara não perder, lembre-se de fazer BACKUP (Exportar) nas configurações.\n\nDeseja continuar mesmo assim?";
  ...
  const analyzeWithGemini = async (base64, mimeType) => {
    setAnalyzing(true);
    setEditingId(null);
    navigateTo('review'); // Usa navigateTo para criar histórico
    
    const apiKey = import.meta.env.VITE_GEMINI_API_KEY;
    const model = "gemini-2.5-flash-preview-09-2025"; // Modelo que funciona
    ...
    try {
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({ contents: [{ parts: [{ text: prompt }, { inlineData: { mimeType, data: base64.split(',')[1] } }] }] })
      });
      ...
      if (!jsonText) throw new Error("IA falhou.");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Volta para o dashboard após salvar (removendo telas de review do histórico idealmente, mas navigateTo resolve o fluxo)
      // Para não "sujar" o histórico, poderíamos voltar 2 vezes se viemos do scan, mas dashboard é seguro.
  ...
  const handleViewExpense = async (exp) => {
    ...
    navigateTo('detail'); // Usa histórico
  ...
  const handleEditStart = async (exp) => {
    ...
    navigateTo('review'); // Usa histórico
  };
  ...
  if (!user) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 p-6 text-center">
        ...
        <Button onClick={handleLogin} className="w-full max-w-xs py-4 text-lg shadow-xl mb-4"><LogIn size={24} /> Entrar com Google</Button>
        
        <button onClick={handleAnonymousLogin} className="text-slate-400 text-sm underline hover:text-slate-600 dark:hover:text-slate-200 transition-colors">Entrar sem conta (Modo Visitante)</button>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6 pb-24">
        <div className="flex justify-between items-center">
          ...
          <button onClick={() => navigateTo('settings')} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300"><Settings/></button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <button onClick={() => { setEditingId(null); navigateTo('scan'); }} className="col-span-2 flex flex-col items-center justify-center p-6 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded-xl border-2 border-blue-100 dark:border-blue-800 active:scale-95 transition-transform"><div className="flex gap-2 mb-2"><Camera size={24} /><Plus size={24} /></div><span className="font-bold text-lg">Nova Despesa</span></button>
    ...
    return (
      <div className="pb-24 space-y-6">
        <div className="flex justify-between items-center sticky top-0 bg-white dark:bg-slate-950 z-10 py-4 border-b dark:border-slate-800">
          <div className="flex gap-3 items-center"><button onClick={goBack} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800"><ChevronRight className="rotate-180 text-slate-900 dark:text-white"/></button><h2 className="text-lg font-bold text-slate-900 dark:text-white">Extrato</h2></div>
  ...
  const renderReview = () => (
    <div className="pb-24 space-y-6">
       <div className="flex gap-3 items-center sticky top-0 bg-white dark:bg-slate-950 z-10 py-4 border-b dark:border-slate-800">
          <button onClick={goBack} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800"><X className="text-slate-900 dark:text-white"/></button>
  ...
  const renderSettings = () => (
     <div className="pb-24 space-y-6">
        ...
        <div className="flex items-center gap-3 p-4 bg-slate-100 dark:bg-slate-900 rounded-xl">
           ...
           <div><div className="font-bold text-slate-900 dark:text-white">{user.displayName || (user.isAnonymous ? 'Visitante' : 'Usuário')}</div><div className="text-xs text-slate-500">{user.email || 'Modo Local'}</div></div>
        </div>
        
        {user.isAnonymous && <div className="bg-yellow-50 text-yellow-800 p-3 rounded-lg text-sm flex gap-2 items-center"><AlertCircle size={16}/> Dados temporários. Exporte ou faça login para salvar.</div>}
        ...
        <div className="text-center text-xs text-slate-400">v3.0 - {user.uid.slice(0,6)}</div>
     </div>
  );
      ...
      <div className="max-w-full h-screen overflow-y-auto p-4">
        ...
        {view === 'scan' && (
            <div className="h-screen flex flex-col items-center justify-center">
               <button onClick={goBack} className="absolute top-4 right-4 p-2"><X/></button>
