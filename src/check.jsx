...
const css = 'w-full rounded-xl border border-slate-200 bg-white p-3 outline-none focus:border-blue-500 dark:border-slate-700 dark:bg-slate-900';
...
export function ReviewScreen({ app, header }) {
  ...
  if (app.analyzing) return <div className="pb-12">{header('Revisar leitura')}<Card className="p-10 text-center"><div className="mx-auto mb-3 h-9 w-9 animate-spin rounded-full border-4 border-slate-200 border-t-blue-600" /><b>Lendo o comprovante…</b><p className="mt-1 text-sm text-slate-500">Campos incertos serão marcados.</p></Card></div>;
  return <div className="pb-12">{header(app.editingId ? 'Editar despesa' : 'Revisar leitura')}
    ...
    <div className="space-y-4"><Card className="p-4"><p className="mb-2 text-sm font-bold">Categoria</p><div className="flex flex-wrap gap-2">{CATEGORIES.map((item)=><button key={item} onClick={()=>set('categoria',item)} className={`rounded-full px-3 py-2 text-sm font-bold ${app.reviewData.categoria===item?'bg-blue-600 text-white':'bg-slate-100 dark:bg-slate-800'}`}>{item}</button>)}</div></Card>
